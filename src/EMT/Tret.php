...
class Tret {
     ...
     * Его можно менять динамически добавляя туда правила с помощью putRule
     *
     * @var unknown_type
     */
    public    $rules;
    ...
    const BASE64_BREAKLINE_TAG = 'YnI='; // br / (с пробелом и слэшем)
     ...
     * @param int $kind
     */
    public function setTagLayoutIfnotset($layout)
    {
    ...
    protected function tag($content, $tag = 'span', $attribute = array())
    {
        ...
        return Lib::buildSafeTag($content, $tag, $attribute,
     ...
     * @param array $params
     */
    public function putRule($name, $params)
    {
     ...
     * @param mixed $value значение
     */
    public function setRule($rulename, $key, $value)
    {
    ...
    public function activate($list,$disable =false, $strict = true)
    {
        ...
        {
            if($disable) $this->disableRule($rulename); else $this->enableRule($rulename);
        }
