...
class Lib
{
     ...
     * <code>
     *  // Remove UTF-8 chars:
     * 	$str = Lib::clearSpecialChars('your text', 'utf8');
     *  // ... or HTML codes only:
     * 	$str = Lib::clearSpecialChars('your text', 'html');
     * 	// ... or combo:
     *  $str = Lib::clearSpecialChars('your text');
     * </code>
     *
     * @param 	string $text
     * @param   mixed $mode
     * @return 	string|bool
     */
    public static function clearSpecialChars($text, $mode = null)
    {
     ...
     * @return  string
     */
    public static function safeTagChars($text, $way)
    {
        if ($way)
            $text = preg_replace_callback('/(\<\/?)(.+?)(\>)/s', create_function('$m','return $m[1].( substr(trim($m[2]), 0, 1) === "a" ? "%%___"  : ""  ) . EMT\Lib::encryptTag(trim($m[2]))  . $m[3];'), $text);
        else
            $text = preg_replace_callback('/(\<\/?)(.+?)(\>)/s', create_function('$m','return $m[1].( substr(trim($m[2]), 0, 3) === "%%___" ? EMT\Lib::decryptTag(substr(trim($m[2]), 4)) : EMT\Lib::decryptTag(trim($m[2])) ) . $m[3];'), $text);
        return $text;
    }
     ...
     * @return 	string
     */
    public static function buildSafeTag($content, $tag = 'span', $attribute = array(), $layout = Lib::LAYOUT_STYLE )
    {
        $htmlTag = $tag;
     ...
     * @return 	string
     */
    public static function encryptTag($text)
    {
     ...
     * @return 	string
     */
    public static function decryptTag($text)
    {
     ...
     * @return string
     */
    public static function htmlCharEntityToUnicode($entity)
    {
    ...
    public static function convert_html_entities_to_unicode(&$text)
    {
        ...
        $text = preg_replace_callback("/\&([a-zA-Z0-9]+)\;/",
            create_function('$m', '$r = EMT\Lib::htmlCharEntityToUnicode($m[1]); return $r ? $r : $m[0];')
            , $text);
    }
}