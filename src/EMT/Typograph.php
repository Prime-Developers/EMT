...
class Typograph extends Base
{
    ...
    public function getOptionsList()
    {
        $arr['all'] = array();
        $bygroup = array();
        foreach($this->allOptions as $opt => $op)
        {
            $arr['all'][$opt] = $this->getOptionInfo($opt);
            $x = explode(".",$opt);
            $bygroup[$x[0]][] = $opt;
        }
        $arr['group'] = array();
        foreach($this->groupList as $group => $ginfo)
        {
            if($ginfo === true)
            {
                $tret = $this->getTret($group);
     ...
     * @return array|false
     */
    protected function getOptionInfo($key)
    {
        if(!isset($this->allOptions[$key])) return false;
        if(is_array($this->allOptions[$key])) return $this->allOptions[$key];

        if(($this->allOptions[$key] == "direct") || ($this->allOptions[$key] == "reverse"))
        {
            $pa = explode(".", $key);
            $tret_pattern = $pa[0];
            $tret = $this->getTret($tret_pattern);
            if(!$tret) return false;
            if(!isset($tret->rules[$pa[1]])) return false;
            $array = $tret->rules[$pa[1]];
            $array['way'] = $this->allOptions[$key];
            return $array;
        }
        return false;
    }
    ...
    {
        if(!isset($this->allOptions[$name])) return;

        // эта настрока связана с правилом ядра
        if(is_string($this->allOptions[$name]))
        {
            $this->set($name, "active", $value );
            return ;
        }
        if(is_array($this->allOptions[$name]))
        {
            if(isset($this->allOptions[$name]['selector']))
            {
                $settingname = "active";
                if(isset($this->allOptions[$name]['setting'])) $settingname = $this->allOptions[$name]['setting'];
                $this->set($this->allOptions[$name]['selector'], $settingname, $value);
            }
        }

        if($name == "OptAlign.layout")
        {
            if($value == "style") $this->setTagLayout(Lib::LAYOUT_STYLE);
            if($value == "class") $this->setTagLayout(Lib::LAYOUT_CLASS);
        }
     ...
     * @return string
     */
    public function fastApply($text, $options = null)
    {
        $obj = new self();
